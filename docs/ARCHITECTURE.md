...
```
Intake → Orchestrator → tasks/YYYYMMDD-orch-<phase>.yml
```
...
### Naming Conventions
- **Role Node IDs**: `role___<company>_<position>_<timeframe>` (e.g., `role_bluestone_mt_2025q3`)
- **Queue Files**: `tasks/YYYYMMDD-orch-<phase>.yml` with top-level `queue:`
